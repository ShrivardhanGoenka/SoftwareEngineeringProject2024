...
import updateWallet from "../APICalls/updateWallet";
...
const Wallet = () => {
  ...
  async function changeBalance() {
    if (action === "") {
      Swal.fire({
        icon: 'info',
        text: 'Please select Add or Withdraw.',
    });
      return null;
    }
    ...
    const res = await updateWallet(action, changeAmount);
  ...
  <div
    key={index}
    className="flex mx-4 mb-2 bg-violet-100 items-start justify-between rounded-md border-[1px] border-[transparent] dark:hover:border-white/20 bg-white px-3 py-[20px] transition-all duration-150 hover:border-gray-200 dark:!bg-navy-800 dark:hover:!bg-navy-700"
  >
    ...
    <div className="flex flex-1 items-center gap-3">
      ...
      <p className="text-sm font-bold">
        S$ {parseFloat(hist.amount).toFixed(2)}
      </p>
    </div>
    </div>
  ...
  return (
    <>
      ...
      {type === "withdraw" && (
        <button
          className={`px-8 py-4 text-white bg-indigo-500 rounded-md shadow-md ${
            action === "withdraw" ? " border-red-500 border-4" : ""
          }`}
          onClick={onClick}
        >
          Withdraw
